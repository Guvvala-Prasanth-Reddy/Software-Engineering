...
from .models import speical_user_access,user_semester
    ...
    if( request.method == 'POST'):
        ...
        if( password == password1):
            if( User.objects.filter(username=username).exists()):
                ...
                user.save()
                new_user = user_semester.objects.create(user_name = username , semester = semester)
                new_user.save()
...
def show_requests(request):
    ...
    print(d[-1])
    resp = User.objects.filter(username = d[-1])
...
def give_access(request):
    ...
    k = k.lstrip("'")
    k = k.rstrip("'")
    u = User.objects.filter(username = k)
    print(u.values('is_staff'))
    if( u.values('is_staff')):
        User.objects.filter(username = h).update(is_superuser = True)
        speical_user_access.objects.filter(user_name = h).delete()
